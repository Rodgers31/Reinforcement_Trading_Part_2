...
Everything (including the CFG overrides) lives inside main() behind the
__main__ guard: importing this module must have zero side effects — the
filename matches pytest's *_test.py collection pattern, so an unguarded
module would start training on mere test collection (PR #1 review, F3).
"""
